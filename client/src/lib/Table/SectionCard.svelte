...
<div class="mb-3">
  ...
  <div
    class="rounded-b-lg shadow border bg-slate-200 p-3 grid grid-cols-1 gap-3"
  >
    {#each $sectionStore.data as section (section.id)}
      <div
        class="card p-3 border-4 border-transparent {section.parentId && 'bg-cyan-50'} {selected?.id == section.id
          ? 'active'
          : ''}"
